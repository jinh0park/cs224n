...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        x_dim = n_features * self.embed_size
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty([x_dim, hidden_size])))
    ...
    def forward(self, w):
        ...
        h = F.relu(x.mm(self.embed_to_hidden_weight) + self.embed_to_hidden_bias[None, :])
        h = self.dropout(h)
        logits = h.mm(self.hidden_to_logits_weight) + self.hidden_to_logits_bias[None, :]

...
if "%1%"=="train" (
    ...
    set CUDA_VISIBLE_DEVICES=0 & python run.py train --save-to=model_kor.bin --train-src=./korean_test/train.en --train-tgt=./korean_test/train.kor --dev-src=./korean_test/dev.en --dev-tgt=./korean_test/dev.kor --vocab=vocab_kor.json --cuda --lr=5e-4 --patience=1 --valid-niter=200 --batch-size=32 --dropout=.3
) else if "%1%"=="test" (
    ...
    set CUDA_VISIBLE_DEVICES=0 & python run.py decode model_kor.bin ./korean_test/test.en ./korean_test/test.kor outputs/test_outputs_kor.txt --cuda
    ...
    python vocab.py --train-src=./korean_test/train.en --train-tgt=./korean_test/train.kor vocab_kor.json
) else (
